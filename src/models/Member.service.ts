...
class MemberService {
    ...
    public async getTopUsers(): Promise<Member[]> {
        const result = await this.memberModel.find({memberStatus: MemberStatus.ACTIVE, memberPoints: { $gte: 1},
    ...
    public async addUsePoint(member: Member, point:number):Promise<Member> {
        const memberId = shapIntoMongooseObjectId(member._id);
        ...
        .findOneAndUpdate(
            {
                _id: memberId,
                memberTypes: MemberType.USER,
                memberStatus: MemberStatus.ACTIVE,
            },
            {$inc: { memberPoinst: point }},
            { new: true }
    ...
    public async getupdatechosenUsers(input: MemberUpdateInput): Promise<Member> {
        input._id = shapIntoMongooseObjectId(input._id);
