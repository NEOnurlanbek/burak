...
restaurantController.processSignup = async (req: AdminRequest, res: Response) => {
    try{
        ...
        console.log("malumotlar", req.body);
        const file = req.file;
...
restaurantController.updatechosenUsers = async (req: Request, res: Response) => {
    try{
        ...
        const result = await memberService.getupdatechosenUsers(req.body);
...
restaurantController.checkAuthSession = async (req: AdminRequest, res: Response) => {
    try{
        ...
        else res.send(`<script> alert("${Message.NOT_AUTHENTOCATED}")</script>`);
    } catch (err) {
...
restaurantController.verifyRestaurant = (req: AdminRequest, res: Response, next: NextFunction) => {
       ...
       if(req.session?.member?.memberType === MemberType.RESTAURANT) {
        ...
        const message = Message.NOT_AUTHENTOCATED;
        res.send(`<script> alert("${message}"); window.location.replace('/admin/login');</script>`);
    }
